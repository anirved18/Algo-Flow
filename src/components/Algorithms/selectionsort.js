  ...
  const handleVisualize = async () => {
    ...
    for (let i = 0; i < arr.length - 1; i++) {
      ...
      for (let j = i + 1; j < arr.length; j++) {
        setCurrentCompare([minIndex, j]); // highlight both bars
        await new Promise((r) => setTimeout(r, speed));

        if (arr[j] < arr[minIndex]) {
          minIndex = j;
          setCurrentCompare([i, minIndex]);
          await new Promise((r) => setTimeout(r, speed));
        }
      }
      ...
      sorted.push(i); // mark element as sorted
      setSortedIndices([...sorted]);
    }

    sorted.push(arr.length - 1); // last element is sorted
    setSortedIndices([...sorted]);
    setCurrentCompare([]);
    setIsSorting(false);
  };
    ...
    <div className="flex flex-col items-center w-full px-4 sm:px-8 mt-[100px]">
      ...
      >
        {array.map((num, idx) => {
          ...
          else if (currentCompare.includes(idx)) bgColor = "bg-red-500"; // comparing both
